...
#' @param d distance between cells
#' @param nSample number of sample in each cell
#' @param mm Number of areas.
#' @param time Maximum time.
#' @param MuxStar0 Mean of unobserved areal level covariates.
#' @param MuxInd0 Mean of unobserved individual level covariates.
...
#'
#'
#' @examples NaiveEstimation(1,5,0.05,2,4,4,20,0,0,1,1,0.3,0.3,0.5,0.5,2.5,0,1,
#' 1,1,1,3,2)
#'
#'
NaiveEstimation=function(ITER,MHiteration,eps,d,nSample,mm,time,MuxStar0,MuxInd0,SigmaxStar0,SigmaxInd0,Sigmav0,Sigmaw0,lambda0,sigma0,delta0,alpha0,beta10,beta20,beta30,beta40,InfPeriod,g){
  ...
  lambda=rep(InfPeriod,n) #########ifected period##############
  I=diag(mm)
  A1=adjacency.matrix(g, g)
  A2=colSums(A1)
  D=-A1
  diag(D)=A2
  ###############################Covaraites without measurement error#######################
  cov1=rnorm(n,0,1)
  cov2=rnorm(mm,0,1)
  ...
  D1=c()
  for(mmm in 1:mm){
    D1[mmm]=sample(N[mmm],1,replace=F)
  }
  D1
  ...
  Nlabel=unlist(Nlabel1)


  Newlabel=matrix(0,n,mm)
  for(mmm in 1:mm){
    Newlabel[,mmm]=rep(D[,mmm],N)
  }
  ...
  data=array(0,c(n,2,ITER))
  for (iter in 1:ITER) {
    xx = c(rep(0,nSample*g*g))
    for (gg in 1:g) {
      x=matrix(0,nSample,g)
      for(i in 1:g){
        x[,i]=runif(nSample,d*(gg-1),d*gg)
      }
      v=nSample*gg*g
      xx[1:v] = c(x)
    }
    ...
    yy = c(rep(0,nSample*g*g))
    for (gg in 1:g) {
      y=matrix(0,nSample,gg)
      for(i in 1:gg){
        y[,i]=runif(nSample,d*(i-1),d*i)
      }
      z=nSample*gg*g
      yy[1:z] = c(y)
    }
        ...
        for(mmm in 1:mm){
          if (Nlabel[i]==mmm){
            if(tau[i]== 0){
              ...
              dx=sum(dx1)
              P=1-exp(-exp(alpha0+beta10*cov1[i]+beta20*cov2[mmm]+beta30*CovX[i]+beta40*CovXStar1[mmm]+phi[mmm])*dx)
              u=runif(1,0,1)
              if(P>u){
                tau[i]=t+1
              }
